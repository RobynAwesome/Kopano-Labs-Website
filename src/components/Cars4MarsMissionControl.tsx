...
export function Cars4MarsMissionControl() {
  return <div className="mission-control">
    <section className="mission-status" aria-label="Cars4Mars mission status">
      ...
      <div className="mission-state-grid">{states.map(([name, state, evidence, tone], index) => <motion.article key={name} className={`mission-state ${tone}`} initial={{opacity:0,y:18}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:index*.05}}>
        <span>{String(index + 1).padStart(2,'0')}</span><b>{name}</b><strong>{state}</strong><p>{evidence}</p>
      </motion.article>)}</div>
    </section>

    <section className="next-gate-panel">
      <div><span className="eyebrow">NEXT PHYSICAL GATE · DFR-01</span><h2>Funding, orders, frame, drive and protected power.</h2><p>Release evidence: approved orders plus loaded forward/reverse, turn and emergency-stop evidence. Design intent does not advance this gate.</p></div>
      <div className="gate-meta"><span>DUE WINDOW</span><strong>03–10 AUG 2026</strong><span>ACCEPTANCE</span><strong>ORDERS + LOADED DRIVE + TURN + E-STOP</strong></div>
    </section>
    ...
    <section className="architecture-section">
      <div className="mission-section-head"><span className="eyebrow">ROVER ARCHITECTURE</span><h2>Bounded intelligence. Deterministic safety.</h2></div>
      <div className="architecture-grid">{architecture.map(([name, spec, rule]) => <article key={name}><span>{name}</span><strong>{spec}</strong><p>{rule}</p></article>)}</div>
    </section>

    <section className="support-section">
      <div className="mission-section-head"><span className="eyebrow">SUPPORT THE MISSION</span><h2>Move a gate, not a vanity metric.</h2><p>Every contribution has to terminate in equipment, funding, expertise, facilities, documentation or learning that advances a recorded gate.</p></div>
      <div className="support-grid">{support.map(([lane, ask, returnState]) => <article key={lane}><span>{lane}</span><h3>{ask}</h3><p>{returnState}</p></article>)}</div>
    </section>
