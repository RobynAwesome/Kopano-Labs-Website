...
if (parts.length !== expectedParts) throw new Error(`DFR-01 source incomplete: expected ${expectedParts} chunks, got ${parts.length}`);

const encoded = (await Promise.all(parts.map((name) => readFile(join(sourceDir, name), 'utf8')))).join('').replace(/\s+/g, '');
if (encoded.length !== expectedBase64Length) throw new Error(`DFR-01 base64 length mismatch: expected ${expectedBase64Length}, got ${encoded.length}`);
